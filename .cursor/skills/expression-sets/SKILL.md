...
Compound ramp uplift (each ramp segment's price uplifts on the *prior segment's* factor, not the list price) is a **group-ramp + pricing-procedure** capability. Five prerequisites, the compounding math, unsupported cases, inspect/research commands, and Help-snapshot grounding:
[`compound-ramp-uplift.md`](compound-ramp-uplift.md).
