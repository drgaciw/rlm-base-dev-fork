...
## Converting to a scheduled cloud run

This file is self-contained on purpose. To run on a schedule, point a cloud
agent's prompt at: *"Run the `build-pde` command in this repo"* (or paste this
file's body). Each run provisions a fresh `pde<datetime>` org, so scheduled
builds never collide and the branch is never mutated.
